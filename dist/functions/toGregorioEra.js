...
const toGregorioEra = (japaneseEra) => {
    try {
        ...
        const parsedDate = (0, date_fns_1.parse)(yyyymmdd, 'yyyy-MM-dd', new Date());
        ...
        if (startYear && startMonth && startDate) {
            const startDateString = startYear.toString().padStart(4, '0') +
                '-' +
                startMonth.toString().padStart(2, '0') +
                '-' +
                startDate.toString().padStart(2, '0');
            const parsedStartDate = (0, date_fns_1.parse)(startDateString, 'yyyy-MM-dd', new Date());
            if ((0, date_fns_1.compareAsc)(parsedDate, parsedStartDate) < 0) {
                throw new Error(era + ' ' + year + '年' + month + '月' + date + '日は存在しません。');
            }
        }
        // 入力された日付が終了日よりも後の場合はエラーを返す
        if (endYear && endMonth && endDate) {
            const endDateString = endYear.toString().padStart(4, '0') +
                '-' +
                endMonth.toString().padStart(2, '0') +
                '-' +
                endDate.toString().padStart(2, '0');
            const parsedEndDate = (0, date_fns_1.parse)(endDateString, 'yyyy-MM-dd', new Date());
            if ((0, date_fns_1.compareAsc)(parsedDate, parsedEndDate) > 0) {
                throw new Error(era + ' ' + year + '年' + month + '月' + date + '日は存在しません。');
            }
        }
        return {
